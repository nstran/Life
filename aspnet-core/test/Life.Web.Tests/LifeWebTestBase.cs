...
{
    public abstract class LifeWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static LifeWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(LifeWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<LifeDbContext> action)
        {
            using (var context = IocManager.Resolve<LifeDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<LifeDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<LifeDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<LifeDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<LifeDbContext, Task<T>> func)
        {
            T result;
